    ...
    <transition name="slide">
        <div id="chat" class="chatRoom">
            ...
            <div class="chat_nav">
                ...
                <div class="comment-wrapper clearfix" @click="goToComment(staticChatFriendObj)">
                    <!-- <img
                        ...
                        <li class="clearfix chatListItem" :data-friend="item.friend" :data-vocieUnread="item.vocieUnread+''" :data-messageType="item.type" ref="item" :class="{'friend':item.friend,'mine':!item.friend}" :key="index" v-for="(item,index) in componentChatList">
                            <div v-if="item.type==1" class="message_wrapper">
                                <div class="person_box">
                                    ...
                                    <img onclick="return false" :src="staticChatFriendObj.headimgurl?staticChatFriendObj.headimgurl:'https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1534938165134&di=f3ae0420c8c174149ac1c123230a28ed&imgtype=0&src=http%3A%2F%2Fmmbiz.qpic.cn%2Fmmbiz_png%2FJCRXU6oUw5s17jKllv9icrTmXvozYWQDeWFhKgEXbYeR9JOEKkrWLjibU7a7FAbsBHibVKca5wWzEiaXHWSgaSlgbA%2F640%3Fwx_fmt%3Dpng'" alt class="avatar" v-if="item.friend" />
                                    ...
                                    <div v-else>
                                        <div>
                                            <div class="myGifoInfo" :class="{'friendPanel':item.friend,'minePanel':!item.friend}" v-if="item.from==userInfo.openid">
                                                <p class="giftRecord_test giftText">{{item.isBeFriendModel?"恭喜成为好友,店长":userInfo.nickname}}送一份精美礼物</p>
                                                <div class="gift">
                                                    <div class="giftImg">
                                            ...
                                            <div class="myGifoInfo" :class="{'friendPanel':item.friend,'minePanel':!item.friend}" v-else>
                                                <p class="giftRecord_test giftText">{{item.isBeFriendModel?"恭喜成为好友,店长":staticChatFriendObj.nickname}}送一份精美礼物</p>
                                                <div class="gift">
                                                    <div class="giftImg">
                                                        <img onclick="return false" v-if="item.chatExtMsg.image" :src="item.chatExtMsg.image" alt />
                                                        <img onclick="return false" class="giftImg" v-else-if="item.name==='beer'" src="../../assets/image/beer.png" alt />
                                                ...
                                                <div>
                                                    <p v-if="!item.isBeFriendModel" class="giftRecord_test giftText">
                                                        <span @click="respondForGift(item,false,item.isBeFriendModel)" class="no">拒收</span>
                                                        <span @click="respondForGift(item,true,item.isBeFriendModel)" class="yes">感谢</span>
                                                    </p>
                                    ...
                                    <div v-else>
                                        <div>
                                            ...
                                            <p v-else class="giftRecord_test no_received">
                                                ...
                                                <span @click="rejectForGame(item)" class="noGame">免战</span>
                                                <span @click="respondForGame(item)" class="yesGame">应战</span>
                                            </p>
                                        </div>
                                    </div>
                                </div>
...
export default {
    ...
    activated () {
        ...
        this.sendingTimes = sessionStorage.getItem(this.staticChatFriendObj.openid)
    ...
    deactivated () {
        ...
        if (this.$route.name != "comment" && this.$route.name != "commentUser") {
            this.setChatFriend({}); //清除vuex里面保存的聊天好友对象
        }
        console.log(
            "this.staticChatFriendObj-----------clear",
            this.staticChatFriendObj
        );
        sessionStorage.removeItem("friendInfo"); //清除缓存中对应的好友信息，避免每次进入聊天页面都是同一个好友
        this.endCursor = null;
    ...
    methods: {
        ...
        _handleChatList (item, nowTime) {
            ...
            var tempObj = {
                ...
                isBeFriendModel: item.chatExtMsg ? item.chatExtMsg.isBeFriendModel : false,
                from: item.from,
                chatMsgID: item.id,
                ...
                return new Promise((resolve, reject) => {
                    ...
                    this.componentChatList.forEach(item => {
                        switch (item.chatExtMsg ? item.chatExtMsg.name : "") {
                            case "beer":
                                item.chatExtMsg.name = "虚拟礼物:啤酒";
                                break;
                            case "flower":
                                item.chatExtMsg.name = "虚拟礼物:鲜花";
                                break;
                            case "house":
                                item.chatExtMsg.name = "虚拟礼物:跑车";
                                break;
                            case "car":
                                item.chatExtMsg.name = "虚拟礼物:邮轮";
                                break;
                            default:
                                break;
                        }
