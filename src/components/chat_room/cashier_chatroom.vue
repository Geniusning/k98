        ...
        <div class="name">
          ...
          <div class="online_status">
            <img
              ...
              v-if="staticChatFriendObj.onlineDiceServer || staticChatFriendObj.onlineL98Server"
              class="online_dot"
            >
            <span
              v-if="staticChatFriendObj.onlineDiceServer || staticChatFriendObj.onlineL98Server"
              class="friendStatus"
            >{{staticChatFriendObj.isInDoor?"店内":"店外"}}</span>
            <span
              v-if="staticChatFriendObj.deskCode && (staticChatFriendObj.onlineDiceServer || staticChatFriendObj.onlineL98Server)"
              class="roomNum"
            >{{`${staticChatFriendObj.deskCode}`}}</span>
          </div>
        </div>
            ...
            >
              <div v-if="item.type===1" class="message_wrapper">
                <div class="person_box">
                  ...
                  <img
                    onclick="return false"
                    :src="staticChatFriendObj.headimgurl?staticChatFriendObj.headimgurl:clientImg"
                    alt=""
                    class="avatar"
                    v-if="item.friend"
                  >
                  <img
                    onclick="return false"
                    :src="userInfo.headimgurl?userInfo.headimgurl:clientImg"
                    alt=""
                    class="avatar"
                    v-else
                  >
              ...
              <div v-else-if="item.type===3" class="message_wrapper cashier_wrapper">
                ...
                <div
                  ...
                  v-if="item.userCouponInfo"
                  class="couponInfo"
                  :class="{'vipmbg':item.userCouponInfo.coupon.type=='月卡券','viptbg':item.userCouponInfo.coupon.type=='次卡券' }"
                >
                  <div class="myleft">
                    <p class="discount_type_text">{{item.userCouponInfo.coupon.type}}</p>
                    <p
                      class="yuE"
                      v-show="item.userCouponInfo.usingTimes>0"
                    >剩{{item.userCouponInfo.usingTimes}}次</p>
                  </div>
                  <div
                    class="mycenter"
                    style="color:#FDDC69"
                    v-show="item.userCouponInfo.coupon.type=='月卡券' ||item.userCouponInfo.coupon.type=='次卡券'"
                  >
                    <div class="discount_theme clearfix">
                      <div
                        class="theme"
                      >{{item.userCouponInfo.coupon.theme?item.userCouponInfo.coupon.theme:"新人礼包"}}</div>
                      <div
                        class="receiver_wrapper"
                        v-if="item.userCouponInfo.coupon.senderHeadImage"
                      >
                        <img
                          onclick="return false"
                          :src="item.userCouponInfo.coupon.senderHeadImage"
                          alt=""
                          class="receiver_avartar"
                        >
                      ...
                      <div class="receiverProject_wrapper">
                        ...
                        <img
                          onclick="return false"
                          v-if="item.userCouponInfo.coupon.image"
                          :src="item.userCouponInfo.coupon.image"
                          class="project_img"
                        >
                      </div>
                    </div>
                    <div class="discount_content">
                      ...
                      <span
                        v-if="item.userCouponInfo.usingTimes != 0"
                    ...
                    <div class="discount_limitAndTime">
                      <div
                        class="limit"
                      >积分:{{item.userCouponInfo.coupon.integral}} &nbsp;&nbsp;&nbsp;{{item.userCouponInfo.coupon.limit}}</div>
                      <p
                        class="time"
                      >{{item.userCouponInfo.coupon.startTime}}至{{item.userCouponInfo.coupon.endTime}}</p>
                    </div>
                  </div>
                  <div
                    class="mycenter"
                  ...
                  <div class="myright">
                    <div v-if="isCashierFlag">
                      ...
                      <p
                        ...
                        style="color:red"
                        v-if="item.isHandle"
                      >{{item.checkPeople?item.checkPeople:userInfo.nickname}}</p>
                    </div>
                    <div v-else>
              ...
              <div v-else-if="item.type===5" class="account_statement">
                <div class="account_wrapper">
                  ...
                  <div class="account_line">
                    ...
                    <span class="cardname">{{item.usercouponname}}</span>
                  </div>
                  <div class="account_line">
...
export default {
  ...
  methods: {
    ...
    async sendAlreayPayMoney() {
      ...
      this.input_value = "已付款";
      this.send();
    },
    //显示收款码
    async showCheckQrCode() {
      ...
      api
        ...
        .then(res => {
          return new Promise((resolve, reject) => {
            ...
            console.log("收银聊天信息-----------", this.resultMessList);
            var i;
            if (!this.isLoadMore) {
            ...
            this.componentChatList.forEach(msg => {
              if (msg.userCouponInfo) {
                msg.userCouponInfo.coupon.codeNum =
                  util.prefixZero(msg.userCouponInfo.coupon.type, 1) +
                  "-" +
                  util.prefixZero(msg.userCouponInfo.coupon.batch, 3) +
                  "-" +
                  util.prefixZero(msg.userCouponInfo.coupon.acquireNum, 7);
                msg["canIuse"] = util.compareDate(
                  msg.userCouponInfo.coupon.startTime,
                  util.timestampToTime(new Date().getTime()).slice(0, 10)
                );
                msg.userCouponInfo.coupon.name = util.returnDiscountContent(
                  msg.userCouponInfo.coupon
                );
                msg.userCouponInfo.coupon.type = util.returnDiscountType(
                  msg.userCouponInfo.coupon.type
                );
              }
            });
            console.log("收银聊天记录-------------", this.componentChatList);
            resolve();
          });
        })
        .then(() => {
    ...
    send() {
      ...
      let messObj = {
        to: this.isCashierFlag
          ...
          : this.staticChatFriendObj.CashierID,
  ...
  watch: {
    cashierLastChatMsg: function(newValue) {
      ...
      if (messageInfo.type === 5) {
        api.loadSelfPayByID(messageInfo.selfpayid).then(res => {
          console.log("selfpayInfo---", res);
          if (res.errCode === 0) {
            messageInfo["selfpayinfo"] = res.info;
          }
        });
      } else {
        messageInfo["selfpayinfo"] = null;
      }
      this.componentChatList.push({
