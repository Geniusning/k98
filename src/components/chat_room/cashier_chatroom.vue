			...
			loadCashierChatList() {
				...
				api.loadCashierChatMsg(data).then(res => {
						return new Promise((resolve, reject) => {
							...
							console.log("收银聊天信息-----------", resultMessList);
							var i;
							if (!this.isLoadMore) {
								for (i = resultMessList.length - 1; i >= 0; i--) {
									let msg = resultMessList[i];
							...
							this.componentChatList.forEach(msg => {
								if (msg.userCouponInfo) {
									...
									msg['canIuse'] = util.compareDate(msg.userCouponInfo.coupon.startTime, util.timestampToTime(new Date().getTime()).slice(0, 10))
									msg.userCouponInfo.coupon.name = util.returnDiscountContent(msg.userCouponInfo.coupon)
									msg.userCouponInfo.coupon.type = util.returnDiscountType(msg.userCouponInfo.coupon.type);
								}
							})
