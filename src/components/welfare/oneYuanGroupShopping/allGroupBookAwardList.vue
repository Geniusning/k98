<template>
    <div class="all-opened-group">
        ...
        <header v-if="allGroupBookList.length">
            ...
            <span class="desc">就差你了，马上加入</span>
        </header>
        <main v-if="allGroupBookList.length">
            <ul class="group-list">
                <li class="group-info" v-for="(item,index) in allGroupBookList" :key="index">
                    ...
                    <span class="PersonsNums">还差{{item.groupBookAwardInfo.personNums-item.joinGroupBookAwardInfo.headImgUrls.headImgUrls.length}}人</span>
                    ...
                    <span class="arrow" @click="goToDetail(item.joinGroupBookAwardInfo.id)">&gt;</span>
                </li>
            </ul>
        </main>
        ...
        loadAllOpenGroupList () {
            api.loadAllOpenGroupList(this.groupBookAwardId).then(res => {
                ...
                if (res.errCode === 0) {
                    ...
                    this.allGroupBookList = joinGroupBookAwardInfoArr.filter(item => {
