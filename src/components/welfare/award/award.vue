<template>
	<div class="content">
        <my-header title="抽奖活动"></my-header>
		<div class="conts">
			<div class="box">
				<div class="pointer" @click.once="chou()">
					<p>立即</p>
					<p>抽奖</p>
				</div>
...
export default {
  ...
  methods: {
    chou() {
      ...
      _this.indexa = "";
      _this.indexb = "";
      _this.winnum = Math.floor(
        Math.random() * Number(_this.prize_list.length - 1)
      );
    ...
    setSan() {
      ...
      for (var i = 0; i < _this.prize_list.length; i++) {
        _this.prize_list[i].troter =
          "rotate(" +
          Math.floor(360 * 100 / _this.prize_list.length) /
            100 *
            (0.5 + Number(i)) +
          "deg) translateX(-50%)";
    ...
    game_over() {
      ...
      if (_this.prize_list[_this.winnum].prizeType == 0) {
        ...
        _this.prize_list[_this.winnum].prizeType == 2
      ) {
        obj = {
          type: 1,
          words1: "恭喜您！",
