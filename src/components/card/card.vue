    ...
    <div class="discount_wrapper">
      ...
      <ul class="no_user_list" v-show="tagIndex==0">
        ...
        <li v-else class="item" v-for="(item,index) in unusedList" :key="index">
          <h3 class="time" style="text-align:center">获得时间：{{item.myGetTime}}</h3>
          <div class="card-info" :class="{'vipmbg':item.coupon.type=='月卡券','viptbg':item.coupon.type=='次卡券' }">
            <div class="myleft">
      ...
      <ul class="usered_list" v-show="tagIndex==1">
        ...
        <li v-else class="item" v-for="(item,index) in usedList" :key="index">
          <h3 class="time" style="text-align:center">核销时间：{{item.useTime}}</h3>
          <div class="card-info" :class="{'vipmbg':item.coupon.type=='月卡券','viptbg':item.coupon.type=='次卡券' }">
            <div class="myleft">
              <p class="discount_type_text">{{item.coupon.type}}</p>
            </div>
      ...
      <ul class="past_list" v-show="tagIndex==2">
        ...
        <li v-else class="item" v-for="(item,index) in timeOutList" :key="index">
          <h3 class="time" style="text-align:center;">过期时间：{{item.coupon.endTime}} 23:59:59</h3>
          <div class="card-info">
            <div class="myleft">
              <p class="discount_type_text">{{item.coupon.type}}</p>
            </div>
      ...
      <ul class="songLiList" v-show="tagIndex==3">
        ...
        <li v-else class="item" v-for="(item,index) in songLiList" :key="index">
          <h3 class="time" style="text-align:center">赠送时间：{{item.sendTime}}</h3>
          <div class="card-info">
            <div class="myleft">
              <p class="discount_type_text">{{item.coupon.type}}</p>
            </div>
  ...
  methods: {
    ...
    distributeDiscount (resDiscountList, discountList) {
      ...
      resDiscountList.forEach(element => {
        ...
        element['myGetTime'] = util.timestampToTime(element.getTime)
        element['useTime'] = util.timestampToTime(element.useTime) ? util.timestampToTime(element.useTime) : 0
        element['sendTime'] = util.timestampToTime(element.sendTime) ? util.timestampToTime(element.sendTime) : 0
    ...
    .no_user_list {
      ...
      .item {
        .card-info {
          display: flex;
          width: 100%;
          background-image: url(../../assets/image/discount_bg.png);
    ...
    .songLiList {
      ...
      .item {
        .card-info {
          display: flex;
          width: 100%;
          background-image: url(../../assets/image/songli_discount_bg.png);
          background-repeat: no-repeat;
          background-size: 100%;
        }
      }
