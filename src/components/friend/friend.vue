  ...
  <div id="friend"
    ...
    <div class="stack-wrapper">
      <div class="switchBtn_wrapper"
        ...
        <p class="intro_soulText"
           v-show="isFirstLoad">一键找到你的有缘人</p>
        <p class="intro_picAndComment"
           v-show="isFirstLoad">照片及评价</p>
      </div>
      <!-- isFirstLoad -->
      ...
      <stack ref="stack"
             ...
             @firstData="listenFirstdata">暂时没有好友</stack>
    ...
    <div class="control_wrapper">
      <div class="soulBtns_wrapper"
           v-show="isEndResultSearchBtnBox">
        <button class="btn"
                @click="cancleSoulSearch">退出</button>
        <button class="btn"
                @click="intoSetting">去完善</button>
      </div>
      ...
      <x-dialog v-model="showToast"
        ...
        <div class="select_wrapper">
          ...
          <div class="dis_wrapper">
            <h3>籍贯</h3>
            <ul class="dis_list">
              <li @click="chooseHomeTownType(index)"
                  :class="{active:homeTownType == index}"
                  v-for="(item,index) in homeTownList"
                  :key="index"
                  v-if="homeTownType!=2">{{item.name}}</li>
              <li style="width:300px;background:#fff;"
                  v-if="homeTownType===2">
                <el-cascader v-if="homeTownType===2"
                             size="small"
                             :options="options"
                             v-model="selectedHomeTownOptions"
                             @change="handleChangeHomeTown"></el-cascader>
              </li>
            </ul>
          </div>
          <div class="dis_wrapper">
    ...
    <div class="bg"
         v-if="isShow_bg"
         @click="share">
      <img class="share-arrow"
           onclick="return false"
           src="../../assets/image/share.png"
           alt />
      <p class="shareText"
         style="top: 3rem;">分享好友，引荐1位朋友加入社群</p>
      <p class="shareText"
         style="top: 4rem;">获得一朵<img class="flower"
             src="../../assets/image/flowerCounts.png" />，红花越多排名越靠前</p>
    </div>
    <!-- <keep-alive> -->
    <topUp v-show="isGiftPanel"
           @closeIntegralPanel="closeIntegralPanel"
           :isInDoor="isInDoor"
           :friendId="friendId"
           :fatherPanelIndex="fatherPanelIndex"></topUp>
    <!-- </keep-alive> -->
    <qrCode v-show="qrIsShow"
            title="您还不是会员,关注享有会员特权"></qrCode>
    <transition name="appear">
      <envelope v-show="isShowEnvelope"
                :text="envelopeText"></envelope>
    </transition>
    <popup-picker v-if="showPopupPickerPos"
                  :show="showPopupPickerPos"
                  :data="positionList"
                  v-model="positionArr"
                  @on-hide="onHide_P"
                  @on-change="onChange_P"></popup-picker>
