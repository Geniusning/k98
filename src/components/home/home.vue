...
<script type='text/ecmascript-6'>
  ...
  export default {
    ...
    computed: {
      ...mapState(["l98Setting", "AdvertisingPhoto", "loadFriendSexType", "baseUrl", "friendList", "inAndOutFriendCursor", "userInfo", "shareUrl", "shopSettingInfo", "noCouponsFlag"]),
    ...
    methods: {
      addHandler() {
        ...
        window.addEventListener("popstate", (e) => {
          if (window.history.state.url == "http://es6.ruanyifeng.com" || window.history.state.url == "https://www.baidu.com") {
          console.log("---------------window.history.state.url------------------------------", window.history.state.url)
    ...
    watch: {
      ...
      friendList: function(newValue) {
        ...
        if (newValue.length < 6) {
          ...
          console.log("this.friendIconList------", this.friendIconList)
          return
        }
