          ...
          <div class="welfare_content">
            <ul class="welfare_list" v-if="friendLeagleList.length">
              <li class="item clearfix" v-for="(item,index) in friendLeagleList" :key="index">
                ...
                <div class="center">
                  <p class="title" style="font-weight:900">{{item.storename}}</p>
                  <p class="desc">{{item.res.name}}</p>
                  <p class="limit">{{item.res.limit}}</p>
                  <p class="price">
                    <span class="discount_p" style="font-size:.3rem">优惠券内容摘要：{{item.res.content}}</span>
                  </p>
                </div>
                <div class="right">
...
<script type='text/ecmascript-6'>
  ...
  export default {
    ...
    methods: {
      //拉取友商派送的优惠券
      getAllianceCoupon() {
        api.getAllianceCoupon(this.resId).then(res => {
      ...
      goToFriShop(shopInfo) {
        ...
        window.location.href = `${shopInfo.url}/k98/home?resId=${shopInfo.res.id}`
      },
      //拉取友商物品
      loadAlliance() {
