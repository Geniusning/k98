 ...
 * @Last Modified time: 2018-08-18 17:50:54
 */
...
let api = {};
let tk = "exjkcmYFV7TItjpcK-bDH28Q2Z9riyLOWvPT7Zkk4InzGhXLI5RPlmAwfAWuX_Sdz8n-2A=="
    //获取用户信息
...
api.createOrder = function() {
    return new Promise((resolve, reject) => {
        axios.get(Url.commonUrl + `/api/createOrder?tk=${tk}`)
            .then(res => {
                if (res.status == 200) {
                    resolve(res.data)
                }
...
api.clearFirstLoadTag = function() {
    ...
    // 获取游戏房间ID
api.createRoom = function(gamePath) {
        return new Promise((resolve, reject) => {
            axios.get(`http://llwant.test.qianz.com:3002/createRoom?tk=${tk}&gamePath=${gamePath}&arenaID=dc1dc692-0abf-4754-b856-806d4e3bc3c2`)
                .then(res => {
                    if (res.status == 200) {
                        resolve(res.data)
                    }
