...
let api = {};
//拉取友商物品
api.getAllianceCoupon = function (resID) {
    return new Promise((resolve, reject) => {
        axios.get(Url.commonUrl + `/api/allianceCoupon?tk=${Url.tk}&resID=${resID}`).then((res) => {
            // console.log(res)
            if (res.status == 200) {
                resolve(res.data)
            }
...
api.queueCombatReply = function (p) {
    return new Promise((resolve, reject) => {
        axios.post("https://singledog.qianz.com/" + `game/dice/queueCombatReply?tk=${Url.tk}`, p).then(res => {
            if (res.status == 200) {
                resolve(res.data)
            }
        }).catch(err => {
