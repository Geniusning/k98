    ...
    <div class="no-open" v-if="!groupShopInfo.joinGroupShopInfo">
      ...
      <div class="oneYuan-header">
            ...
            <CountDown v-else style="color:red" @finish="countDownFinish" :time="time">
              <template #default="timeData" format="DD 天 HH:mm:ss">
                ...
                <span class="block">{{ timeData.minutes<10?'0'+timeData.minutes:
                    timeData.minutes}}</span> <span class="colon">:</span>
                <span class="block">{{ timeData.seconds<10?'0'+timeData.seconds:timeData.seconds
                    }}</span> </template> </CountDown> </div> </div> </div> <div class="oneYuan-content">
            ...
            <div class="oneYuan-opened-countdown">
              ...
              <CountDown style="display:inline-block;color:red" @finish="countDownFinish" :time="time">
                <template #default="timeData" format="DD 天 HH:mm:ss">
                  ...
                  <span class="block">{{ timeData.seconds<10?'0'+timeData.seconds:timeData.seconds
                      }}</span> </template> </CountDown> </div> <div class="oneYuan-opened-headImgUrlList">
                      <ul class="list">
                        <li class="item" v-for="(img,index) in groupShopInfo.joinGroupShopInfo.headImgUrls.headImgUrls"
                          :key="index">
        ...
        <div class="oneYuan-rule">
          ...
          <div v-if="time">
            <button v-if="groupShopInfo.isJoinGroupShop === true && groupShopInfo.joinGroupShopInfo.status "
              class="oneYuan-btn" @click="isShow_bg=true">分享好友,邀请TA拼团</button>
            <button @click="openGroupShopping(groupShopInfo.joinGroupShopInfo?groupShopInfo.joinGroupShopInfo.status:false)"
              v-else-if="groupShopInfo.joinGroupShopInfo?groupShopInfo.joinGroupShopInfo.status===false:false"
              class="oneYuan-btn">我要开团(支付100积分预订团购指标)</button>
            <button @click="openGroupShopping(groupShopInfo.joinGroupShopInfo?groupShopInfo.joinGroupShopInfo.status:false)"
              v-else-if="groupShopInfo.isJoinGroupShop === false" class="oneYuan-btn">
...
export default {
  ...
  mounted() {
    ...
    this.groupShopInfoId = shareGroupShopId
      ? shareGroupShopId
      : this.$route.query.groupShopInfoId
    this.openGroupId = shareOpenGroupId
      ? shareOpenGroupId
      : this.$route.query.openGroupId
  ...
  methods: {
    ...
    openGroupShopping(flag) {
      if (this.groupShopInfo.joinGroupShopInfo && flag) {
        //已开团，则参团
        api
          .joinGroupShopping(
            this.groupShopInfo.joinGroupShopInfo.id,
            this.groupShopInfoId
          )
          .then(res => {
    ...
    loadGroupShopForUser() {
      api
        .loadGroupShopForUser(this.groupShopInfoId, this.openGroupId)
        .then(res => {
          ...
          if (res.errCode === 0) {
            setTimeout(() => {
              let shareObj = {
                title: '1元拼团',
                desc: '一起来抢劫老板啰...',
                link: `${this.shareUrl}#/oneYuan?visitType=11&groupShopInfoId=${
                  this.groupShopInfoId
                }&openGroupId=${this.openGroupId}&openId=${
            ...
            let joinGroupOpenIds = this.groupShopInfo.joinGroupShopInfo
              ? this.groupShopInfo.joinGroupShopInfo.openId.openId
              : []
            ...
            if (this.groupShopInfo.joinGroupShopInfo) {
              this.time =
                this.groupShopInfo.joinGroupShopInfo.interval * 1000 -
                new Date().getTime()
              if (this.time <= 0) {
                this.time = 0
              }
