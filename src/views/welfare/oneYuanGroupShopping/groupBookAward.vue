      ...
      <div class="oneYuan-header">
              ...
              <CountDown style="display:inline-block;color:red" @finish="countDownFinish" :time="isCountingDownTime">
                <template #default="timeData" format="DD 天 HH:mm:ss">
                  ...
                  <span class="block">{{ timeData.seconds<10?'0'+timeData.seconds:timeData.seconds
                      ...
                      <ul class="list">
                        <li class="item" v-for="(img,index) in groupBookAward.joinGroupBookAwardInfo.headImgUrls.headImgUrls"
                          :key="index">
        ...
        <div class="oneYuan-rule">
          ...
          <div v-if="isCountingDownTime">
            <button v-if="groupBookAward.isJoining === true && groupBookAward.joinGroupBookAwardInfo.status "
...
export default {
  ...
  mounted() {
    ...
    this.groupBookAwardId = shareGroupShopId
      ? shareGroupShopId
      : this.$route.query.groupBookAwardId
  ...
  methods: {
    ...
    openGroupBookAward(flag) {
      try {
        if (this.groupBookAward.joinGroupBookAwardInfo && flag) {
          //已开团，则参团
          api
            .joinGroupBookAward(
              this.groupBookAward.joinGroupBookAwardInfo.id,
              this.groupBookAwardId
            )
            .then(res => {
          ...
          api
            .openGroupBookAward(
              this.groupBookAwardId,
              this.groupBookAward.groupBookAwardInfo.personNums
            )
            .then(res => {
    ...
    loadGroupBookAwardForUser() {
      api
        .loadGroupBookAwardForUser(this.groupBookAwardId, this.openGroupId)
        .then(res => {
          ...
          if (res.errCode === 0) {
            setTimeout(() => {
              let shareObj = {
                ...
                link: `${
                  this.shareUrl
                }#/pinTuan?visitType=11&groupBookAwardId=${
                  this.groupBookAwardId
            ...
            let joinGroupOpenIds = this.groupBookAward.joinGroupBookAwardInfo
              ? this.groupBookAward.joinGroupBookAwardInfo.openId.openId
              : []
            if (joinGroupOpenIds.indexOf(this.userInfo.openid) > -1) {
              this.isJoinGroup = true
            }
            if (this.groupBookAward.joinGroupBookAwardInfo) {
              this.isCountingDownTime =
                this.groupBookAward.joinGroupBookAwardInfo.interval * 1000 -
                new Date().getTime()
