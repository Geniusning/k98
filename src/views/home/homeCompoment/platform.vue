<template>
  <div class="flatform-wrapper">
    <div class="flatform-tab vux-1px-b">
      <img @click="showSelectDialog=true" class="select-icon" src="../../../assets/image/select-platform.png"
        alt="">
    ...
    <div v-if="showSelectDialog" class="dialog-demo">
      <div class="select_wrapper">
        <img onclick="return false" src="../../../assets/image/close-round.png" alt class="close"
          @click="cancel" />
    ...
    <popup-picker style="visibility:hidden" class="popup" :show="showPopupPickerPos" :data="industryList"
      v-model="industryArr" @on-hide="onHide" @on-change="onChange"></popup-picker>
  </div>
</template>
<script>
import {
...
import { PopupPicker, XDialog, TransferDomDirective as TransferDom } from 'vux'
...
export default {
  ...
  methods: {
    handleProp(platformInfo) {
      ...
      this.resultShowPlatform = this.resultShowPlatform.sort(
        util.sortByKeyS2L('distance')
      )
    },
    //num 个数，
    randNum(min, max, num) {
    ...
    search() {
      ...
      let result = copyPlatformList.filter((item, index) => {
        ...
        if (this.keyword) {
          return (
            storeInfo.name.indexOf(this.keyword) > -1 &&
            storeInfo.city === this.city &&
            this.industry === storeInfo.industryType
          )
        } else {
          return (
            storeInfo.city === this.city &&
    ...
    async goToLeagleHome(info) {
      ...
      if (res === 0) {
        window.location.href = `${info.storeInfo.url}/#/home`
      }
    },
    async goToLeagleFri(info) {
      let res = await this.collectFrequentPlatform(info.storeInfo.storeId)
      if (res === 0) {
        window.location.href = `${info.storeInfo.url}/#/friend`
      }
    },
