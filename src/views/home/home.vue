<template>
  <div id="home" class="home">
    ...
    <div class="home-top_wrapper" v-show="shopSettingInfo.shopModeId !== FOUR_VIDEO">
      ...
      <swiper :show-dots="false" :auto="true" class="slider" :loop="true" :list="demo01_list"
        v-model="demo01_index"></swiper>
    ...
    <div class="video-wrapper" v-else-if="shopSettingInfo.shopModeId === FOUR_VIDEO">
      ...
      <Video :currentVideoId="currentVideoId" v-if="videoList.length>0 && !showVideoList"
      ...
      <div class="content">
            ...
            <div class="desc_wrapper">
              <div class="lineOne">
                <ul class="fri_list" v-show="friendIconList1.length>=3">
                  <li class="item" :class="'avar'+index" v-for="(item,index) in friendIconList1"
                    :key="index">
              ...
              <div class="linetwo">
                <ul class="fri_list">
                  <li class="item" :class="'avar'+index" v-for="(item,index) in friendIconList2"
                    :key="index">
                    <img onclick="return false" :src="item.headimgurl?item.headimgurl:defaultAvatarImg"
                      class="min_avatar" />
                  </li>
        ...
        <div v-else-if="shopSettingInfo.shopModeId === ONE_BUSINESS" class="entertainment_wrapper2">
          <div class="entertainment_top">
            <div class="left">
              <img onclick="return false" class="k_1" @click="goToFriendForSvip" src="../../assets/image/k_1.png"
                alt="">
            </div>
            <div class="right" @click="gotoFriend">
              <div class="lineOne">
                <ul class="fri_list" v-show="friendIconList3.length>=3">
                  <li class="item" style="margin-left:-0.13rem" :class="'avar'+index" v-for="(item,index) in friendIconList3"
                    :key="index">
                    <img style="width:.5rem;height:.5rem" onclick="return false" :src="item.headimgurl?item.headimgurl:defaultAvatarImg"
                      class="min_avatar" />
                  </li>
          ...
          <div class="entertainment_bottom">
            ...
            <div class="right">
              <img onclick="return false" @click="gotoPlay" class="k_1" src="../../assets/image/k_4.png"
                alt="">
            </div>
          </div>
        </div>
        ...
        <div class="welfare_wrapper" v-show="recommentList.length">
          <div class="title_content_wel vux-1px-b">
            <div class="title clearfix">
              ...
              <h2 class="shop_title" @click="closeWebPage">{{shopSettingInfo.shopModeId===
                ZERO_COMMON ? "店长推荐" :"会员产品推荐" }}</h2>
              <span v-if="shopSettingInfo.shopModeId === ZERO_COMMON" class="desc">积分兑换更优惠</span>
            </div>
            <div class="more fr">
              <span @click="goToMoreRecommend" v-if="shopSettingInfo.shopModeId !== THREE_SHOPPING">更多&gt;&gt;</span>
              <!-- <img onclick="return false" src="../../assets/image/fuli.png" alt class="letter" v-show="noCouponsFlag" @click="toWelfare" /> -->
            </div>
          </div>
          <div class="welfare_content" v-if="recommentList.length && THREE_SHOPPING != shopSettingInfo.shopModeId">
          ...
          <vipProductList :vipProductList="recommentList" v-else></vipProductList>
        </div>
        <!-- 分享引导 -->
        ...
        <div class="shotBtn_wrapper">
          <img @click="gotoVip" onclick="return false" class="shotImg" src="../../assets/image/integral.png"
            alt />
          <img @click="gotoCardList" onclick="return false" class="shotImg" src="../../assets/image/cardShot.png"
            alt />
        </div>
      </div>
    </div>
    ...
    <topUp :convertType="convertType" v-if="isGiftPanel" @closeIntegralPanel="closeIntegralPanel"
      :giftInfo="recommendItemIndo" :fatherPanelIndex="fatherPanelIndex"></topUp>
...
export default {
  ...
  mounted() {
    ...
    this.gameUrl = `https://${window.location.host}/`
    if (this.resId) {
  ...
  activated() {
    setTimeout(() => {
      let shareObj = {
        ...
        link: `${this.shareUrl}#/home?visitType=3&phone=${
          this.userInfo.phone
  ...
  methods: {
    ...
    goToFriShop(shopInfo) {
      ...
      api.statAlliCount(data).then(res => {
        ...
        window.location.href = `${shopInfo.url}/#/home?resId=${
