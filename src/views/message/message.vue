          ...
          <li v-if="!isClientListFlag" class="item vux-1px-b" @click="ChatToClient">
            <div class="info_message">
              ...
              <div class="name_and_message">
                ...
                <div class="warmTims-btn" @click.stop="showTips = true">温馨提示</div>
          ...
          <li v-if="isClientListFlag" class="item vux-1px-b" @click="clientChat(item)" v-for="(item,index) in clientServiceList" :key="index">
            <div class="info_message">
              ...
              <div class="name_and_message">
                ...
                <p class="captainMessage">{{userInfo.role?item.lastMsg.content:"欢迎光临! 有任何问题或建议，请留言"}}</p>
                <p class="time">
                  {{item.lastMsg?item.lastMsg.stime.slice(8,10)==today?item.lastMsg.stime.slice(10,16):item.lastMsg.stime.slice(5,10):""}}</p>
              </div>
            </div>
          </li>
          ...
          <li class="item vux-1px-b" v-for="(item,index) in captainMessageList" :key="index">
            <div class="info_message">
              <div class="avatar">
                ...
                <img v-else-if="item.type===10" src='../../assets/image/11.png' style="border-radius:0" alt="">
...
export default {
  ...
  beforeRouteUpdate(to, from, next) {
    ...
    if (
      from.name === 'clientChat' ||
      from.name === 'chat' ||
      from.name === 'cashierChat'
    ) {
      ...
      api.loadClientServiceList(phone).then(res => {
        ...
        if (res.CliSerID && !res.uerInfos) {
          ...
          if (this.clientObj.lastMsg) {
            this.clientObj.lastMsg.stime = util.timestampToTime(
              res.lastMsg.stime
            )
          }
        } else {
          ...
          if (tempArr.length > 0) {
            tempArr.forEach((client, index) => {
              ...
              if (client.lastMsg) {
                ...
                client.lastMsg.stime = util.timestampToTime(
                  client.lastMsg.stime
                )
              }
    ...
    _loadMutualEvents() {
      api.loadMutualEvents().then(res => {
        if (res.errCode === 0) {
          ...
          tempEventList = tempEventList.concat(
